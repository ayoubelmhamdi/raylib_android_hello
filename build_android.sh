...
# ------- CONFIG -------------
sdk="/data/sdk"
PLATFORMS=$sdk/android/platforms
...
BUILD_TOOLS=$sdk/android/build-tools/29.0.3
TOOLCHAIN=$sdk/android/ndk/toolchains/llvm/prebuilt/linux-x86_64
NATIVE_APP_GLUE=$sdk/android/ndk/sources/android/native_app_glue
...
for ABI in $ABIS; do
    ...
    # TODO: MANUAL SELECT FILES:
	# Compile project
	for file in src/*.c; do
		$CC -c $file -o "$file".o $INCLUDES -I$TOOLCHAIN/sysroot/usr/include/$CCTYPE $FLAGS $ABI_FLAGS
	done
...
$BUILD_TOOLS/aapt package -f -m \
	...
	-I $PLATFORMS/android-29/android.jar

# Compile NativeLoader.java
javac -verbose -source 1.8 -target 1.8 -d android/build/obj \
	-bootclasspath jre/lib/rt.jar \
	-classpath $PLATFORMS/android-29/android.jar:android/build/obj \
	-sourcepath src android/build/src/com/${APP_COMPANY_NAME}/${GAME_NAME}/R.java \
	android/build/src/com/${APP_COMPANY_NAME}/${GAME_NAME}/NativeLoader.java
...
$BUILD_TOOLS/aapt package -f \
	...
	-I $PLATFORMS/android-29/android.jar -F game.apk android/build/dex
...
jarsigner -keystore android/${PROJECT_NAME}.keystore -storepass ${APP_KEYSTORE_PASS} -keypass ${APP_KEYSTORE_PASS} \
	-signedjar game.apk game.apk ${PROJECT_NAME}Key
